...
function buildMarkerDescriptor(marker: Marker): string {
  ...
  marker.terrainFeatures.forEach(tf => { if (tf.feature) parts.push(tf.feature) })
  return parts.join(', ')
}
...
function MarkerRow({ marker }: { marker: Marker }) {
  ...
  return (
    <p className="cc-ref-entry">
      ...
      {descriptor && <>{descriptor}{marker.effect ? '. ' : '.'}</>}
      {marker.effect}
    </p>
  )
}
...
function RefSection({ title, color, children }: { title: string; color: string; children: React.ReactNode }) {
  return (
    <div className="cc-ref-section">
      <p className="cc-ref-heading" style={{ color }}>{title}</p>
      ...
      <div className="cc-back-body">
        {hasMarkers && (
          <RefSection title="Markers" color={faction.color}>
            {card.markers.map((m: Marker) => <MarkerRow key={m.id} marker={m} />)}
          </RefSection>
        )}
        {hasTokens && (
          <RefSection title="Tokens" color={faction.color}>
            {card.tokens.map((t: Token) => <TokenRow key={t.id} token={t} />)}
          </RefSection>
        )}
      </div>
