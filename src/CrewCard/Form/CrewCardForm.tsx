...
const TOKEN_PRESETS: Record<string, string> = {
  Abandoned:
    'During the end phase, if this model does not have more models (other than itself) it considers friendly within 3" of it than models it considers enemy, this model is dealt 2 irreducible damage. Then, if this token did not damage this model, remove this token.',
  Adaptable:
    "Before performing a duel, this model may remove this token to add a suit of its choice to its duel total.",
  Adversary:
    "Friendly models receive a + to attack actions targeting this model. During the end phase, remove this token.",
  "Aetheric Surge":
    "This model receives +2 Skl and a suit of its choice when declaring attack actions.",
  Analyzed:
    "This model may not reduce damage with its abilities. Remove this token during the end phase.",
  "Aura (Binding)":
    'Enemy models within 6" may not empower duels. During the end phase, remove this token.',
  "Aura (Concealment)":
    'The area within 2" of this model is concealing terrain. Friendly models may choose to be unaffected by this terrain. During the end phase, remove this token.',
  "Aura (Fire)":
    'The area within 2" of this model is hazardous (Burning) terrain. Friendly models may choose to be unaffected by this terrain. During the end phase, remove this token.',
  "Aura (Fumes)":
    'Enemy models within 3" receive a - to attack actions that target friendly models. During the end phase, remove this token.',
  "Aura (Hazardous)":
    'The area within 2" of this model is hazardous terrain. Friendly models may choose to be unaffected by this terrain. During the end phase, remove this token.',
  "Aura (Negligent)":
    'Enemy models within 2" of this model that declare a non-Walk action must pass a TN 10 Wp duel or the action fails. During the end phase, remove this token.',
  "Aura (Poison)":
    'The area within 2" of this model is hazardous (Poison) terrain. Friendly models may choose to be unaffected by this terrain. During the end phase, remove this token.',
  "Aura (Staggered)":
    'When an enemy model within 2" of this model activates, it must discard a card or gain a Staggered token.',
  Backtrack:
    "When performing a duel, this model may remove this token before any player cheats fate. If it does so, all cards in the conflict are discarded without effect and the duel is performed again with the same modifiers. This token may not be used if there are any jokers in the conflict.",
  Badge:
    "When this model would be killed, it may discard a card and remove this token to heal 2.",
  Balm: "During the end phase, this model heals 1. Then, remove this token.",
  Blight:
    'This model suffers a - to Wp duels generated by Plague models. During the end phase, enemy models within 1" gain a Blight token, then if there are no other models within 1" with a Blight token, remove this token.',
  "Bog Spirit":
    "After this model is killed, the crew that applied this token may summon a Will o' the Wisp with 3 health into base contact.",
  Bolstered:
    "This model receives +1 to its Df and Wp. During the end phase, remove this token. Canceled by Injured.",
  Brilliance:
    "When this model cheats fate, the crew that applied this token may remove it to deal 1 damage to this model or heal it 2.",
  Broodling:
    "After this model is killed, summon a friendly Terror Tot into base contact.",
  Burning:
    "During the end phase deal 1 damage to this model and enemy models in base contact with it. Then remove this token.",
  Challenged:
    "This model must discard a card to target any model other than the one that applied this token. During the end phase, remove this token.",
  ...
  Drift:
    'When this model activates, it may remove this token to place into base contact with a friendly Tide marker within 5".',
  Entranced:
    "This model's actions that target a friendly model cannot be cheated. After this model resolves an action targeting a friendly model, remove this token.",
  Explosive:
    'A model with an Explosive token may take the Interact action to remove the token and make a friendly Strategy marker within 1", and not within 4" of another friendly Strategy marker. Explosive tokens may not be removed in any other way.',
  Exposed:
    "This model cannot gain cover. During the end phase, remove this token. Canceled by Hidden.",
  Familia:
    'When a friendly model within 6" is dealt damage, this model may remove this token to reduce that damage by 1.',
  ...
  Flicker:
    "During the end phase, kill this model. This token cannot be removed.",
  Focused:
    "Before performing a duel, this model may remove this token to receive a + to the duel. Canceled by Distracted.",
  "Fragile Ego":
  ...
  Glowy:
    "Attacks that target this model are unaffected by concealing terrain. During the end phase, remove this token.",
  Glutted:
    "When this model ends its activation, it may remove this token to heal 1.",
  ...
  Hastened:
    "This model receives +2 Sp. When this model ends its activation, remove this token. Canceled by Staggered.",
  Hidden:
    "This model gains cover. During the end phase, remove this token. Canceled by Exposed.",
  Hunger:
    'When this model ends its activation, it must deal 1 damage to another of its allies within 1" to remove this token, or deal 1 damage to itself.',
  Impact:
    "When this model succeeds in an attack action that deals damage, it must remove this token to deal +1 damage.",
  "Improvised Part":
    "This model may declare tactical actions printed on the stat cards of non-master allies within 6\" as if they were printed on this model's stat card. After declaring such an action, remove this token.",
  Incurable: "This model cannot heal. During the end phase, remove this token.",
  Injured:
    "This model suffers -1 to its Df and Wp. During the end phase, remove this token. Canceled by Bolstered.",
  Insight:
    "Before performing a duel, this model may remove this token to look at the top card of its fate deck and may discard it.",
  Instinct:
    "When this model succeeds in an attack action, it may remove this token to receive an additional raise.",
  "Interesting Parts":
    "This model is affected by the enemy crew card as if it were a unique model allied to the enemy leader, but it may not affect or use power bars. This model considers enemy non-Scheme, non-Strategy markers to be friendly.",
  ...
  Shame:
    "This model cannot be targeted by the actions of other enemy models. When this model ends its activation, it may choose to deal 1 damage to itself to remove this token.",
  Shielded:
    "When this model is dealt non-irreducible damage, it must remove this token to reduce that damage by 1. This token may reduce damage to 0.",
  ...
  "Spiritual Chains":
    "When this model flips, cheats, or discards the black joker, it is dealt 2 damage and then this token is removed.",
  Staggered:
...
export default function CrewCardForm({ card, onChange }: Props) {
  ...
  const removeAbilityGroup = (groupId: string) =>
    update({
      abilityGroups: card.abilityGroups.filter((g) => g.id !== groupId),
    });
  ...
  const updateAbility = (
    ...
    const g = card.abilityGroups.find((g) => g.id === groupId)!;
    updateAbilityGroup(groupId, {
      abilities: g.abilities.map((a) =>
        a.id === abilityId ? { ...a, ...patch } : a,
      ),
    });
  };
  ...
  const removeAbility = (groupId: string, abilityId: string) => {
    const g = card.abilityGroups.find((g) => g.id === groupId)!;
    updateAbilityGroup(groupId, {
      abilities: g.abilities.filter((a) => a.id !== abilityId),
    });
  };
  ...
  const updateActionGroup = (groupId: string, patch: Partial<ActionGroup>) =>
    update({
      actionGroups: card.actionGroups.map((g) =>
        g.id === groupId ? { ...g, ...patch } : g,
      ),
    });
  ...
  const updateTriggerGroup = (groupId: string, patch: Partial<TriggerGroup>) =>
    update({
      triggerGroups: card.triggerGroups.map((g) =>
        g.id === groupId ? { ...g, ...patch } : g,
      ),
    });
  ...
  const removeTriggerGroup = (groupId: string) =>
    update({
      triggerGroups: card.triggerGroups.filter((g) => g.id !== groupId),
    });

  const updateGroupTrigger = (groupId: string, patch: Partial<Trigger>) =>
    update({
      triggerGroups: card.triggerGroups.map((g) =>
        g.id === groupId ? { ...g, trigger: { ...g.trigger, ...patch } } : g,
      ),
    });
  ...
  const updateTerrainFeature = (
    ...
    updateMarker(markerId, {
      terrainFeatures: m.terrainFeatures.map((tf) =>
        tf.id === tfId ? { ...tf, feature } : tf,
      ),
    });
  };
  ...
  const removeTerrainFeature = (markerId: string, tfId: string) => {
    const m = card.markers.find((m) => m.id === markerId)!;
    updateMarker(markerId, {
      terrainFeatures: m.terrainFeatures.filter((tf) => tf.id !== tfId),
    });
  };
        ...
        {card.abilityGroups.map((group, gi) => (
          ...
          >
            <GroupHeader
              ...
              excludePeon={group.excludePeon}
              excludeSummonToken={group.excludeSummonToken}
