    ...
    <div className="ability-entry">
      ...
      <div className="row gap-sm">
        <label className="inline">
          <input type="checkbox" checked={ability.defensiveSymbol !== ''}
            onChange={e => onChange({ defensiveSymbol: e.target.checked ? '🛡️' : '' })} />
          Defensive?
        </label>
        {ability.defensiveSymbol !== '' && (
          <select value={ability.defensiveSymbol} onChange={e => onChange({ defensiveSymbol: e.target.value })}>
      ...
      <input value={ability.requirement} onChange={e => onChange({ requirement: e.target.value })} placeholder="Requirement, e.g. Once per activation. (optional)" />
