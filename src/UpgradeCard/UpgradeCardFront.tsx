...
function triggerPreamble(actionType: TriggerActionType, statCard: boolean, count: number): string {
  ...
  let spec: string
  switch (actionType) {
    case 'attack': spec = 'all its attack actions'; break
    case 'all':   spec = statCard ? 'all actions printed on its stat card' : 'all actions'; break
    default:       spec = `its ${actionType} actions`
  }
  ...
  return (
    <div className="upgrade-card">
      ...
      <div className="uc-content">
        <h1 className="uc-name" style={{ color: faction.color }}>{card.name || 'UPGRADE NAME'}</h1>
        <div className="uc-ornament" style={{ color: faction.color }}>~ ❦ ~</div>
        ...
        <div className="uc-body">
          ...
          {hasAbilities && (
            <div className="uc-block">
              <p className="uc-preamble">This model gains the following {abilityNoun}:</p>
              {card.abilities.map(a => <AbilityRow key={a.id} ability={a} />)}
            </div>
          )}

          {hasTriggers && (
            <div className="uc-block">
              <p className="uc-preamble">{triggerPreamble(card.triggerActionType, card.triggerPrintedOnStatCard, card.triggers.length)}</p>
              {card.triggers.map(t => <TriggerRow key={t.id} trigger={t} />)}
            </div>
          )}

          {hasActions && (
            <div className="uc-block">
              <p className="uc-preamble">This model gains the following {actionNoun}:</p>
        ...
        <div className="uc-limitations">
          <p className="uc-lim-heading" style={{ color: faction.color }}>LIMITATIONS</p>
